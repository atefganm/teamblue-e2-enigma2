...
def SwapAutostart(reason, session=None, **kwargs):
	global startswap
	if reason == 0:
		print("[SwapManager] autostart", config.plugins.swapmanager.swapautostart.value)
...
class SwapManager(Screen):
	...
	def updateSwap2(self, result=None, retval=None, extra_args=None):
		...
		if config.plugins.swapmanager.swapautostart.value or self.swap_name == _("manufacturer defined swap"):
			self["autostart_off"].hide()
			self["autostart_on"].show()
